...
contract FunkyPlanets is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(
            ...
            "Can only mint the maxMintAmount tokens at a time"
        );
        require(
            supply + _mintAmount <= maxSupply,
            "Purchase would exceed max supply of planets"
        );
        require(
            msg.value >= _cost * _mintAmount,
            "Ether value sent is not correct, check getCostToMint function to get the individual price"
        );
    ...
    function mintReservedPlanets(
        ...
        require(saleIsActive, "Sale is not active");
        require(
            _reserveAmount <= maxMintAmount,
            "Can only mint the maxMintAmount tokens at a time"
        );
        require(
            supply + _reserveAmount <= maxSupply,
            "Purchase would exceed max supply of planets"
        );
        require(
            _reserveAmount > 0 && _reserveAmount <= reservedPlanetsForTeam,
            "Purchase would exceed max reserved supply of planets and should be more than 0"
        );
